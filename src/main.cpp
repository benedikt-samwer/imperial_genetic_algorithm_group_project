...
int main()
{
    ...
    if (mode == "d")
    {
        ...
        optimize(vector_size, circuit_vector.data(), discrete_fitness, discrete_validity, params);
    }
    ...
    else if (mode == "c")
    {
        ...
        optimize(num_units, volume_params.data(), cont_fitness, cont_validity, params);
    }
    ...
    else
    {
        ...
        auto hybrid_fitness = [](int i_size, int* i_vec, int r_size, double* r_vec) -> double
        { return circuit_performance(i_size, i_vec, r_size, r_vec); };

        auto hybrid_validity = [num_units](int i_size, int* i_vec, int r_size, double* r_vec) -> bool
        {
            Circuit c(num_units);
        ...
        optimize(vector_size, circuit_vector.data(), num_units, volume_params.data(), hybrid_fitness, hybrid_validity,
                 params);
    }

    // Calculate performance with optimized values (still silent)
    double performance = circuit_performance(vector_size, circuit_vector.data(), num_units, volume_params.data());

    // Create a circuit object for detailed analysis, still silent
    Circuit circuit(num_units, volume_params.data());
    circuit.initialize_from_vector(vector_size, circuit_vector.data(), volume_params.data());
    ...
    for (int i = 0; i < num_units; i++)
    {
        ...
        else if (mode == "c")
        {
            ...
            // Known-valid discrete circuit (hardcoded)
            const int fixed_circuit[vector_size] = {1, 2, 4, 3, 5, 3, 0, 8, 11, 7, 12, 7, 0, 7, 11, 8, 6, 9, 7, 10, 3};
        ...
        else
        {
            ...
            // Run hybrid optimization (cout is redirected, so no debug output)
            optimize(vector_size, circuit_vector, num_units, volume_params, hybrid_fitness, hybrid_validity, params);
        }

        // Calculate performance with optimized values (still silent)
        double performance = circuit_performance(vector_size, circuit_vector, num_units, volume_params);
        ...
        std::cout << "Final circuit economic value: £" << std::fixed << std::setprecision(2) << performance
                  << " per second\n\n";
        ...
        std::cout << "\nCircuit Performance:\n";
        std::cout << "- Palusznium recovery: " << std::fixed << std::setprecision(2) << palusznium_recovery << "%\n";
        std::cout << "- Palusznium grade: " << std::fixed << std::setprecision(2) << palusznium_grade << "%\n";
        std::cout << "- Gormanium recovery: " << std::fixed << std::setprecision(2) << gormanium_recovery << "%\n";
        std::cout << "- Gormanium grade: " << std::fixed << std::setprecision(2) << gormanium_grade << "%\n";
        ...
        for (int i = 0; i < num_units; i++)
        {
            std::cout << "Unit " << i << ": " << std::fixed << std::setprecision(2) << unit_volumes[i] << " m³\n";
        }
        std::cout << "Total volume: " << std::fixed << std::setprecision(2) << total_volume << " m³\n";

        // Economic analysis
        std::cout << "\nEconomic Analysis:\n";
        double palusznium_value = circuit.get_palusznium_recovery() * 8 * 120;
        double gormanium_value = circuit.get_gormanium_recovery() * 12 * 80;
        std::cout << "- Palusznium revenue: £" << std::fixed << std::setprecision(2) << palusznium_value << "/s\n";
        std::cout << "- Gormanium revenue: £" << std::fixed << std::setprecision(2) << gormanium_value << "/s\n";
        std::cout << "- Total revenue: £" << std::fixed << std::setprecision(2) << (palusznium_value + gormanium_value)
                  << "/s\n";
