...
std::vector<int>
create_varied_circuit(const std::vector<int> &template_vec, int num_units,
...
generate_initial_population(int population_size, int num_units,
                            std::function<bool(int, int *)> validity_check) {
  ...
  for (int i = 0; i < num_units; i++) {
    if (i % 2 == 0) {
      template2[2 * i + 1] =
          num_units; // Even units send concentrate to Palusznium
      template2[2 * i + 2] = num_units + 2; // and tailings to final tailings
    } else {
      template2[2 * i + 1] =
          num_units + 1;        // Odd units send concentrate to Gormanium
  ...
  for (int i = 0; i < num_units; i++) {
    if (i < num_units / 2) {
      template3[2 * i + 1] =
          i + num_units / 2;                // First half feed to second half
  ...
  std::cout << "Generating initial population of valid circuits..."
            << std::endl;

  while (population.size() < population_size && attempts < max_attempts) {
    // Pick a random template
    const auto &tmpl = templates[std::uniform_int_distribution<int>(
    ...
    if (unique_circuits.find(candidate) == unique_circuits.end()) {
      ...
      if (population.size() % 10 == 0) {
        std::cout << "Generated " << population.size() << " valid circuits"
                  << std::endl;
      }
    }
  ...
  std::cout << "Initial population: " << population.size() << " valid circuits"
            << std::endl;
...
int optimize(int int_vector_size, int *int_vector,
  ...
  std::cout << "Initializing population for " << n_units << " units..."
            << std::endl;
  ...
  for (int gen = 0; gen < params.max_iterations; ++gen) {
    ...
    while (next_gen.size() < population.size()) {
      ...
      if (next_gen.size() < population.size() &&
          validity(int_vector_size, c2.data())) {
...
int optimize(int int_vector_size, int *int_vector, int real_vector_size,
  ...
  optimize(int_vector_size, int_vector, wrapped_func_int, wrapped_valid_int,
           params);
  ...
  optimize(real_vector_size, real_vector, wrapped_func_real, wrapped_valid_real,
           params);
