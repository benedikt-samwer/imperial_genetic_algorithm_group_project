...
double circuit_performance(int vector_size, int *circuit_vector,
  ...
  if (!circuit.initialize_from_vector(vector_size, circuit_vector,
                                      unit_parameters, testFlag)) {
    // Invalid structure
    return -1e12;
  }

  // Run the mass balance
  bool converged = circuit.run_mass_balance(
      simulator_parameters.tolerance, simulator_parameters.max_iterations);
  if (!converged) {
...
double circuit_performance(int vector_size, int *circuit_vector,
                           int unit_parameters_size, double *unit_parameters) {
  return circuit_performance(vector_size, circuit_vector, unit_parameters_size,
                             unit_parameters, default_simulator_parameters,
                             false);
}
...
double circuit_performance(int vector_size, int *circuit_vector) {
  ...
  double result =
      circuit_performance(vector_size, circuit_vector, num_parameters, nullptr,
