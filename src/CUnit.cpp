...
CUnit::CUnit()
    : conc_num(0), tails_num(0), mark(false), volume(10.0),
      ...
      conc_palusznium(0.0), conc_gormanium(0.0), conc_waste(0.0), rho(0.0),
      phi(0.0), tails_palusznium(0.0), tails_gormanium(0.0), tails_waste(0.0),
      V_min(2.5), V_max(20.0) {}

CUnit::CUnit(int conc, int tails)
    : conc_num(conc), tails_num(tails), mark(false),
      volume(Constants::Circuit::DEFAULT_UNIT_VOLUME), feed_palusznium(0.0),
      feed_gormanium(0.0), feed_waste(0.0),
      ...
      phi(Constants::Physical::SOLIDS_CONTENT), conc_palusznium(0.0),
      conc_gormanium(0.0), conc_waste(0.0), tails_palusznium(0.0),
      tails_gormanium(0.0), tails_waste(0.0),
      V_min(Constants::Circuit::MIN_UNIT_VOLUME),
      V_max(Constants::Circuit::MAX_UNIT_VOLUME) {}
...
void CUnit::process() {
  /* ----------- 1. Residence time τ ----------- */
  // constexpr double rho   = Constants::Physical::MATERIAL_DENSITY;   // 3000
  // kg/m³ constexpr double phi   = Constants::Physical::SOLIDS_CONTENT; // 0.10
