...
TEST_F(ValidityCheckerTest, BasicValidN10) {
  Circuit c(10);
  std::vector<int> v_corrected = {0, 1,  10, 2,  11, 3,  12, 4,  10, 5, 11,
...
TEST_F(ValidityCheckerTest, SmallValidN4) {
  ...
  std::vector<int> v = {
      1, 2, 3, 0, 3,
      4, 3, 0, 6}; // Corrected: {feed_idx, u0_c,u0_t, u1_c,u1_t, u2_c,u2_t,
...
TEST_F(ValidityCheckerTest, DISABLED_MassBalanceNotConverge) {
  ...
  std::vector<int> cv = {
      0, 0, 1, 1,
      0}; // Example of a problematic circuit (u0<->u1 tight loop without
...
TEST_F(ValidityCheckerTest, InvalidUnitParametersSize) {
  ...
  ASSERT_FALSE(c.check_validity((int)circuit_vec.size(), circuit_vec.data(),
                                (int)unit_params.size(), unit_params.data()));
}
...
TEST_F(ValidityCheckerTest,
       MissingAllTerminals) { // Should fail multiple checks, including this one
  Circuit c(1);               // n=1, P1=1, P2=2, T=3
  std::vector<int> v = {
      0, 0,
      0}; // u0 -> u0, u0 (fails self-loop, same dest, then terminal checks)
  ASSERT_FALSE(c.check_validity((int)v.size(), v.data()));
}
...
TEST_F(ValidityCheckerTest, MissingP1AndP2Terminals) {
  Circuit c(1); // n=1, P1=1, P2=2, T=3
  std::vector<int> v = {
      0, 3, 3}; // u0 -> T, T (fails same dest, then terminal check for P1/P2)
  ASSERT_FALSE(c.check_validity((int)v.size(), v.data()));
}
