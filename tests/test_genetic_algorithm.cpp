...
static const int int_test_answer[21] = {0, 1, 2, 0, 3, 1, 4, 2, 0, 1, 3,
                                        4, 2, 1, 0, 3, 2, 4, 1, 3, 0};
double last_int_error = 0.0;
double last_real_error = 0.0;
static const std::vector<double> real_test_answer = {
    0.8, 0.4, 0.4, 0.8, 0.0, 0.8, 0.6, 0.3, 0.5, 0.9, 0.2,
...
double circuit_performance_mixed_fitness_adapter(int L_discrete,
                                                 int *discrete_vars,
                                                 int L_continuous,
  ...
  if (L_discrete <= 0 || (2 * n_units + 1) != L_discrete ||
      (L_continuous > 0 && L_continuous != n_units)) {
  ...
  if (L_continuous > 0) {
    for (int i = 0; i < L_continuous; ++i) {
      ...
      continuous_penalty +=
          (continuous_vars[i] - 0.5) * (continuous_vars[i] - 0.5);
    }
  }
...
bool actual_validity_mixed_adapter(int L_discrete, int *discrete_vars,
                                   int L_continuous, double *continuous_vars) {
  if (L_discrete <= 0)
    return false;
  int n_units = (L_discrete - 1) / 2;
  if ((2 * n_units + 1) != L_discrete)
  ...
  if (L_continuous > 0 && L_continuous == n_units) {
    return temp_circuit.check_validity(L_discrete, discrete_vars, L_continuous,
                                       continuous_vars);
...
TEST_F(GeneticAlgorithmTest, OptimizeSimpleValidDiscreteCircuit) {
  ...
  ASSERT_GT(result.best_fitness, -1e9)
      << "Fitness should be better than a very small number for a valid "
         "circuit.";
  ...
  ASSERT_TRUE(c_final.check_validity(L_discrete, initial_guess.data()))
      << "GA found an invalid solution.";
...
// adapted.
static const std::vector<double> target_beta_values_for_cont_test = {
    0.5, 0.6}; // For n=2 example
// This fitness function is for the continuous-only GA optimize signature
double simple_continuous_fitness_adapter(int L_continuous,
                                         double *continuous_vars) {
...
TEST_F(GeneticAlgorithmTest, OptimizeSimpleContinuousVariables) {
  ...
  std::cout << "Continuous GA - Best Fitness: " << result.best_fitness
            << " after " << result.generations << " generations." << std::endl;
  std::cout << "Best continuous genome: ";
  for (int i = 0; i < L_continuous; ++i) {
    std::cout << initial_continuous_guess[i]
              << (i == L_continuous - 1 ? "" : ", ");
    ASSERT_NEAR(initial_continuous_guess[i],
                target_beta_values_for_cont_test[i], EPSILON);
  }
  std::cout << std::endl;
}

// New fitness function to match the global real_test_answer (L=21)
double match_real_test_answer_fitness_adapter(int L_continuous,
                                              double *continuous_vars) {
...
TEST_F(GeneticAlgorithmTest, OptimizeContinuousToMatchRealTestAnswerL21) {
  ...
  std::vector<double> initial_guess =
      ga_initial_continuous_L21_vec; // Use the L21 initial vector
  ...
  int status =
      optimize(L_continuous, initial_guess.data(),
  ...
  std::cout << "Continuous GA (match real_test_answer L21) - Best Fitness: "
            << result.best_fitness << " after " << result.generations
...
TEST_F(GeneticAlgorithmTest, OptimizeDiscreteWithN10Circuit) {
  ...
  Circuit c_final(n_units);
  ASSERT_TRUE(c_final.check_validity(L_discrete, initial_guess.data()))
      << "GA found an invalid N10 solution.";
}
...
TEST_F(GeneticAlgorithmTest, OptimizeMixedWithN10CircuitAndL10Betas) {
  ...
  int status = optimize(L_discrete, initial_discrete_guess.data(), L_continuous,
                        initial_continuous_guess.data(),
                        circuit_performance_mixed_fitness_adapter,
                        actual_validity_mixed_adapter, params);
  ...
  bool is_valid = final_circuit.check_validity(
      L_discrete, initial_discrete_guess.data(), L_continuous,
      initial_continuous_guess.data());
