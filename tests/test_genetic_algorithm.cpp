...
static const std::vector<int> ga_initial_discrete_n10_vec = { // L=20, so n should be (20-1)/2 -> not integer. This was for L=21 if int_test_answer was target
                                                            // The old main had L1 = vector1.size() for this, where vector1 had 20 elements.
                                                            // This seems to be a mismatch with int_test_answer that has 21 elements.
                                                            // For n=10, L must be 21. The old vector1 had 20 elements.
                                                            // Let's use a slice of int_test_answer or a new proper n=10 vector.
                                                            // For now, let's define it as it was in the old main's vector1 (L=20) and adjust if needed or make a new L=21 vector.
    // Old vector1 (L=20):
    //  1, 0, 3, 2, 1, 0, 4, 3, 2, 1, 0,
    //  3, 4, 2, 1, 0, 3, 2, 1, 4 // (last element 0 was missing to make it 20 from 21 of int_test_answer's pair)
    // Let's use a valid n=10 initial guess (L=21) for consistency in tests aiming for actual circuit performance.
...
// Note: circuit_performance itself might return negative values (costs > revenue).
double circuit_performance_fitness_adapter(int L_discrete, int* discrete_vars) {
    ...
    // If your circuit_performance or underlying CCircuit needs beta values that are NOT optimized by GA here,
    // they would need to be handled differently (e.g. fixed default betas, or betas set externally).
    return circuit_performance(L_discrete, discrete_vars);
}

double circuit_performance_mixed_fitness_adapter(int L_discrete, int* discrete_vars, int L_continuous, double* continuous_vars) {
    // This adapter is for the GA's mixed optimization function signature.
    // We need to ensure circuit_performance or the underlying Circuit object can use these continuous_vars (betas).
    // This is a placeholder for how one *might* integrate it. The actual CSimulator/CCircuit API will dictate.
    
    // Option 1: If circuit_performance can take beta values (not shown in CSimulator.h example)
    // return circuit_performance(L_discrete, discrete_vars, L_continuous, continuous_vars);

    // Option 2: Create a temporary Circuit object, set betas, then calculate performance.
    // This assumes CCircuit has a way to set betas and then a way to get performance that CSimulator typically wraps.
    ...
    // We must ensure it's called before trying to run mass balance or get economic value.
    // The check_validity function internally calls initialize_from_vector as part of its process if the structure is basic, 
    // but for performance, we need a fully initialized and mass-balanced circuit.

    // A robust way: initialize, set betas (if any), run mass balance, then get economic value.
    // The current circuit_performance(L, v) likely does this internally without exposing beta parameters.
    // For a mixed GA, circuit_performance would need to accept beta values.
    // Lacking that, this fitness function cannot easily use the optimized continuous_vars with the existing circuit_performance.

    // Fallback to discrete-only performance if circuit_performance cannot accept continuous_vars:
    // This makes the continuous part of GA useless for *this specific fitness function*.
    // To truly test mixed optimization, circuit_performance needs to use the continuous variables.
    // For the purpose of making the code compile and run with GA, we'll simulate this.
    // In a real scenario, this is where CSimulator.h/CCircuit.h would need modification.
...
bool actual_validity_discrete_adapter(int L_discrete, int* discrete_vars) {
    if (L_discrete <= 0) return false;
    int n_units = (L_discrete - 1) / 2;
    if ((2 * n_units + 1) != L_discrete) return false;
    Circuit temp_circuit(n_units);
...
protected:
    ...
    void SetUp() override {
        ...
        // params.continuous_mutation_step_size = 0.1; // This field does not exist in Algorithm_Parameters
        params.convergence_threshold = 1e-4;
        params.stall_generations = 20;
    }
};

// Test GA with discrete variables trying to find a known simple valid circuit for n=1
// Circuit: {0, 1, 3} (Feed u0, u0->P1,T). n=1, P1=1, P2=2, T=3.
TEST_F(GeneticAlgorithmTest, OptimizeSimpleValidDiscreteCircuit) {
    const int n_units = 1;
    const int L_discrete = 2 * n_units + 1;
    std::vector<int> initial_guess(L_discrete, 0); // Start with all zeros or random
    // Example: initial_guess = {0,0,0}

    // We use the actual_validity_function and circuit_fitness_function
    int status = optimize(L_discrete, initial_guess.data(), 
                        circuit_performance_fitness_adapter, 
                        actual_validity_discrete_adapter, 
                        params);
    
    ASSERT_EQ(status, 0) << "Optimization failed to complete successfully.";

    OptimizationResult result = get_last_optimization_result();
    ASSERT_GT(result.best_fitness, -1e9) << "Fitness should be better than a very small number for a valid circuit.";
    
    std::cout << "Discrete GA - Best Fitness: " << result.best_fitness 
              << " after " << result.generations << " generations." << std::endl;
...
// For simplicity, let's use a placeholder fitness until a clear example is available.
// The real_test_answer and real_test_function from original can be adapted.
...
double simple_continuous_fitness_adapter(int L_continuous, double* continuous_vars) {
    if (L_continuous != static_cast<int>(target_beta_values_for_cont_test.size())) return -1e18;
...
bool dummy_validity_continuous_adapter(int L_continuous, double* cv) { 
    // Add any specific checks for continuous variables if needed, e.g. range [0,1]
    for(int i=0; i < L_continuous; ++i) {
        if (cv[i] < 0.0 || cv[i] > 1.0) return false;
    }
    return true; 
}
...
TEST_F(GeneticAlgorithmTest, OptimizeSimpleContinuousVariables) {
    ...
    int status = optimize(L_continuous, initial_continuous_guess.data(),
                        simple_continuous_fitness_adapter, 
                        dummy_validity_continuous_adapter, 
                        params);

    ASSERT_EQ(status, 0) << "Optimization failed (continuous variables).";
    OptimizationResult result = get_last_optimization_result();
    ASSERT_NEAR(result.best_fitness, 0.0, EPSILON*EPSILON*L_continuous ) << "Continuous GA did not converge to target fitness.";

    std::cout << "Continuous GA - Best Fitness: " << result.best_fitness << " after " << result.generations << " generations." << std::endl;
...
double match_real_test_answer_fitness_adapter(int L_continuous, double* continuous_vars) {
    if (L_continuous != static_cast<int>(real_test_answer.size())) return -1e18; // Ensure correct length
    double sum_sq_error = 0;
    for (int i = 0; i < L_continuous; ++i) {
...
TEST_F(GeneticAlgorithmTest, OptimizeContinuousToMatchRealTestAnswerL21) {
    ...
    ASSERT_EQ(status, 0) << "Optimization failed (continuous variables to match real_test_answer).";
    OptimizationResult result = get_last_optimization_result();
    ASSERT_NEAR(result.best_fitness, 0.0, EPSILON * EPSILON * L_continuous) << "Continuous GA did not converge to target fitness for real_test_answer.";

    std::cout << "Continuous GA (match real_test_answer L21) - Best Fitness: " << result.best_fitness 
              << " after " << result.generations << " generations." << std::endl;
    std::cout << "Best continuous genome: ";
    for (int i = 0; i < L_continuous; ++i) {
...
TEST_F(GeneticAlgorithmTest, OptimizeDiscreteWithN10Circuit) {
    ...
    std::cout << "Discrete GA (N=10) - Best Fitness: " << result.best_fitness
              << " after " << result.generations << " generations." << std::endl;
...
TEST_F(GeneticAlgorithmTest, OptimizeMixedWithN10CircuitAndL10Betas) {
    ...
    std::vector<double> initial_continuous_guess(L_continuous, 0.5); // Initial betas at 0.5

    params.max_iterations = 200; 
    params.population_size = 200;

    int status = optimize(L_discrete, initial_discrete_guess.data(),
                        L_continuous, initial_continuous_guess.data(),
                        circuit_performance_mixed_fitness_adapter, 
                        actual_validity_mixed_adapter, 
                        params);
    ...
    std::cout << "Mixed GA (N=10) - Best Fitness: " << result.best_fitness 
              << " after " << result.generations << " generations." << std::endl;
    std::cout << "Best discrete genome (N=10): ";
    for (int i = 0; i < L_discrete; ++i) {
    ...
    bool is_valid = final_circuit.check_validity(L_discrete, initial_discrete_guess.data(), L_continuous, initial_continuous_guess.data());
